...
base_path = '/account/tli/carcinogenecity/results/mol2vec/para_selection'
...
def combine_test_probabilities(n):   
    path='/account/tli/carcinogenecity/results/mol2vec/para_selection/probabilities_output'
    
    ###get the seed
    mcc=pd.read_csv('/account/tli/carcinogenecity/data/mol2vec/supervise/mol2vec_' + n + '.csv')
    ...
    tmp = pd.read_csv(join(knn_base_path, 'test_knn_paras_1_K_5.csv'))
    knn = tmp[['id', 'y_true']]
    ...
    tmp = pd.read_csv(join(lr_base_path, 'test_lr_paras_2.csv'))
    lr = tmp[['id', 'y_true']]
    ...
    tmp = pd.read_csv(join(svm_base_path, 'test_svm_paras_10.csv'))
    svm = tmp[['id', 'y_true']]
    ...
    tmp = pd.read_csv(join(rf_base_path, 'test_rf__paras_15.csv'))
    rf = tmp[['id', 'y_true']]
    ...
    tmp = pd.read_csv(join(xgboost_base_path, 'test_xgboost_paras_105.csv'))
    ...
    data.to_csv(join(path+'/test_probabilities_' + str(n) + '.csv'))
    

for n in ['supervised', 'org']:
    combine_test_probabilities(n)