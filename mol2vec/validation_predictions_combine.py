...
def combine_validation_probabilities(n):
    ...
    tmp = pd.read_csv(join(knn_base_path, 'validation_knn_paras_1_K_5.csv'))
    knn = tmp[['id', 'y_true']]
    ...
    tmp = pd.read_csv(join(lr_base_path, 'validation_lr_paras_2.csv'))
    lr = tmp[['id', 'y_true']]
    ...
    tmp = pd.read_csv(join(svm_base_path, 'validation_svm_paras_10.csv'))
    svm = tmp[['id', 'y_true']]
    ...
    tmp = pd.read_csv(join(rf_base_path, 'validation_rf__paras_15.csv'))
    rf = tmp[['id', 'y_true']]
    ...
    tmp = pd.read_csv(join(xgboost_base_path, 'validation_xgboost_paras_105.csv'))
