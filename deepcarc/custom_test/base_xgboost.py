...
def generate_baseClassifiers(tmp, test, data_split, mcc, base_path):
    ...
    X_test, y_test = test[X.columns], test['label']
    ...
    for seed in mcc[mcc.model == 'xgboost']['seed'].unique():
        ...
        ### define column name
        col_name = 'xgboost_'+'seed_'+seed+'_paras_'+var+'_lr_'+str(para[0])+'_n_'+str(para[1])+'_depth_'+str(para[2])+'_subsample_'+str(para[3])+'_scale_pos_weight_'+str(para[4])
