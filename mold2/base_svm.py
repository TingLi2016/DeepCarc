...
def get_cols(corr_df, threshold):
    cols = corr_df[corr_df.correlation > threshold].feature.unique()
    return cols 
...
tmp = pd.read_csv('/account/tli/carcinogenecity/data/mold2/carcinogenicity_mold2_330.csv',low_memory=False)
cols = tmp.columns[10:]
...
base_path = '/account/tli/carcinogenecity/results/mold2/para_selection/svm'
...
for i in range(200):
    ...
    for train_index, validation_index in skf.split(X, y):
        ...
        ### define column name 
        col_name = 'svm_'+'seed_'+str(i)+'_skf_'+str(j)+'_paras_'+var+'_kernel_'+para[0]+'_C_'+str(para[1])+'_gamma_'+str(para[2])+'_weight_'+str(para[3])
        col_name1 = 'svm_'+'seed_'+str(i)+'_paras_'+var+'_kernel_'+para[0]+'_C_'+str(para[1])+'_gamma_'+str(para[2])+'_weight_'+str(para[3])
