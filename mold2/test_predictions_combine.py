...
base_path = '/account/tli/carcinogenecity/results/mold2/para_selection'
...
def combine_test_probabilities(n):   
    path='/account/tli/carcinogenecity/results/mold2/para_selection/probabilities_output'
    
    ###get the seed
    mcc=pd.read_csv('/account/tli/carcinogenecity/data/mold2/supervise/mold2_' + n + '.csv')
    ...
    tmp = pd.read_csv(join(knn_base_path, 'test_knn_paras_0_K_3.csv'))
    knn = tmp[['id', 'y_true']]
    ...
    tmp = pd.read_csv(join(lr_base_path, 'test_lr_paras_0.csv'))
    lr = tmp[['id', 'y_true']]
    ...
    tmp = pd.read_csv(join(rf_base_path, 'test_rf__paras_5.csv'))
    rf = tmp[['id', 'y_true']]
    ...
    tmp = pd.read_csv(join(xgboost_base_path, 'test_xgboost_paras_89.csv'))
