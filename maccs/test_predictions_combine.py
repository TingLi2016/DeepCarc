...
def combine_test_probabilities(n):   
    ...
    tmp = pd.read_csv(join(knn_base_path, 'test_knn_paras_3_K_9.csv'))
    knn = tmp[['id', 'y_true']]
    ...
    tmp = pd.read_csv(join(lr_base_path, 'test_lr_paras_1.csv'))
    lr = tmp[['id', 'y_true']]
    ...
    tmp = pd.read_csv(join(svm_base_path, 'test_svm_paras_5.csv'))
    svm = tmp[['id', 'y_true']]
    ...
    tmp = pd.read_csv(join(rf_base_path, 'test_rf__paras_51.csv'))
    rf = tmp[['id', 'y_true']]
    ...
    tmp = pd.read_csv(join(xgboost_base_path, 'test_xgboost_paras_87.csv'))
    ...
    data.to_csv(join(path+'/test_probabilities_' + n + '.csv'))
    

for n in ['supervised', 'org']:
    combine_test_probabilities(n)
#combine_test_probabilities('supervised')
